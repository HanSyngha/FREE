...
worklogRoutes.post('/external', async (req, res) => {
  try {
    ...
    const user = await prisma.user.findUnique({ where: { loginid } });
    if (!user) {
      res.status(404).json({ error: `사용자를 찾을 수 없습니다: ${loginid}. 먼저 웹에서 로그인하세요.`, link: 'https://3.39.170.84.nip.io:6090' });
      return;
    }
    if (!user.teamId) { res.status(400).json({ error: `해당 사용자(${loginid})의 팀이 배정되지 않았습니다.`, link: 'https://3.39.170.84.nip.io:6090' }); return; }
    if (!user.groupId || !user.partId) { res.status(400).json({ error: `해당 사용자(${loginid})의 그룹/파트 설정이 필요합니다.`, link: 'https://3.39.170.84.nip.io:6090' }); return; }
    ...
    res.json({ success: true, items: createdItems, count: createdItems.length, message: `${createdItems.length}건의 업무가 추가되었습니다.`, link: 'https://3.39.170.84.nip.io:6090' });
  } catch (error) {
